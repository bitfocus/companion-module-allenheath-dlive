...
/**
 * The first 8 scenes are reserved utility scenes and cannot be recalled
 * (matches the recallScene action's dropdown)
 */
const FIRST_RECALLABLE_SCENE_INDEX = 8

/**
 * Creates a scene recall preset (MixRack targets only)
 * @param sceneIndex 0-based scene index (8 = scene 9, the first recallable scene)
 */
function createSceneRecallPreset(sceneIndex: number): CompanionButtonPresetDefinition {
	const sceneNum = sceneIndex + 1 // 1-based for display
	return {
		type: 'button',
		category: 'Scenes',
		name: `Recall Scene ${sceneNum}`,
		style: {
			text: `Scene\\n${sceneNum}`,
			size: 14,
			color: 0xffffff,
...
/**
 * Creates a cue list recall preset (Surface targets only)
 * @param recallId Recall ID (displayed 0-based, matching the console)
 */
function createCueRecallPreset(recallId: number): CompanionButtonPresetDefinition {
	return {
		type: 'button',
		category: 'Cue Lists',
		name: `Recall Cue ID ${recallId}`,
		style: {
			text: `Cue\\n${recallId}`,
			size: 14,
			color: 0xffffff,
...
/**
 * Creates a Go/Next/Previous scene control preset (Surface targets only).
 * The CC number and value must be edited to match the console's
 * Utility / Control / MIDI settings - the console defines them, not the protocol.
 */
function createGoNextPreviousPreset(name: string, buttonText: string): CompanionButtonPresetDefinition {
	return {
		type: 'button',
		category: 'Scene Control',
		name: `${name} (set CC to match console MIDI settings)`,
		style: {
			text: buttonText,
			size: 18,
			color: 0xffffff,
		...
		steps: [
			{
				down: [
					{
						actionId: 'goNextPrevious',
						options: { controlNumber: 0, controlValue: 0 },
					},
				],
				up: [],
			},
...
/**
 * Creates a MIDI transport (MMC) preset for a transport command
 */
function createMidiTransportPreset(commandId: number, label: string): CompanionButtonPresetDefinition {
	return {
		type: 'button',
		category: 'Transport',
		name: `Transport ${label}`,
		style: {
			text: label.replace(' (Punch In)', '').replace(' (Punch Out)', ''),
			size: 14,
			color: 0xffffff,
			bgcolor: 0x333333,
		},
		steps: [
			{
				down: [
					{
						actionId: 'midiTransport',
						options: { transport: commandId },
					},
				],
				up: [],
			},
 ...
 * Scene, cue and Go/Next/Previous presets follow the configured connection target,
 * matching the action gating (scene recall is MixRack only; cue list recall and
 * Go/Next/Previous are Surface only). When no config is available, all are offered.
 * @param instance Module instance
 */
export function UpdatePresets(instance: ModuleInstance): void {
	const presets: CompanionPresetDefinitions = {}
	const target = instance.config?.target
	...
	// Scene recall presets (MixRack only)
	if (target !== 'surface') {
		for (let sceneIndex = FIRST_RECALLABLE_SCENE_INDEX; sceneIndex < SCENE_COUNT; sceneIndex++) {
			presets[`scene_recall_${sceneIndex + 1}`] = createSceneRecallPreset(sceneIndex)
		}
	}

	// Cue list recall and Go/Next/Previous presets (Surface only)
	if (target !== 'mixrack') {
		for (let recallId = 0; recallId < CUE_LIST_COUNT; recallId++) {
			presets[`cue_recall_${recallId}`] = createCueRecallPreset(recallId)
		}
		presets.scene_control_go = createGoNextPreviousPreset('Scene Go', 'GO')
		presets.scene_control_next = createGoNextPreviousPreset('Scene Next', 'NEXT')
		presets.scene_control_previous = createGoNextPreviousPreset('Scene Previous', 'PREV')
	}

	// MIDI transport presets (both targets)
	for (const choice of MMC_COMMAND_CHOICES) {
		presets[`midi_transport_${choice.id}`] = createMidiTransportPreset(choice.id as number, choice.label)
	}
